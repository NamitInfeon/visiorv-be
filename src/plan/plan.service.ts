...
export class PlanService {
  ...
  async create(obj: PlanDTO) {
    try {
      ...
      // Reload with relations using QueryBuilder to bypass cache
      const savedDataWithRelations = await this.planRepo
        .createQueryBuilder("plan")
        .leftJoinAndSelect("plan.plan_category", "plan_category")
        .where("plan.id = :id", { id: savedData.id })
        .getOne();

      return {
        success: true,
        message: "Plan created successfully",
        data: savedDataWithRelations,
      };
    } catch (error) {
      return {
        success: false,
  ...
  async update(id: number, obj: PlanDTO) {
    try {
      // First, validate that plan exists
      ...
      // Use update query builder to ensure clean update
      await this.planRepo.update({ id }, obj);

      // Reload the plan with relations using QueryBuilder to bypass cache
      const updatedPlanWithRelations = await this.planRepo
        .createQueryBuilder("plan")
        .leftJoinAndSelect("plan.plan_category", "plan_category")
        .where("plan.id = :id", { id })
        .getOne();
